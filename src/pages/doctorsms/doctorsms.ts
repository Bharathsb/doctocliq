...
@IonicPage()
@Component({
  selector: 'page-doctorsms',
  templateUrl: 'doctorsms.html',
})
export class DoctorsmsPage {
registerForm
numberresend=0
  ...
  resendsms(){
    ...
    if(this.numberresend==0||this.numberresend==1){
      ...
      let seq = this.api.post(this.api.apidoctors+this.api.apiresend_sms, body)  
  ...
  validatecode(){
     if (this.validate()) {
      ...
      let seq = this.api.post(this.api.apidoctors+this.api.apiconfirm, body)  
      seq.map(res => res.json()).subscribe(res => {
          this.shared.hideLoading()
          console.log(res)
          // if(res.patient) { 
            this.navCtrl.push(DoctorthanksPage);
